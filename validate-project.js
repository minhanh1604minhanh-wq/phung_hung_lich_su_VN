...
const dataPath='public/data/phung-hung.json';
let profile;
...
check('Slug nhân vật',profile.id==='phung-hung');
...
check('Nhập vai đúng nhân vật',profile.roleplay?.learnerRole?.vi?.includes('Phùng Hưng'));
check('NPC nhập vai đúng cố vấn',profile.roleplay?.npcRole?.vi?.includes('cố vấn'));
check('Khóa xưng hô Phùng Hưng',profile.roleplay?.addressRule?.vi?.includes('Không bao giờ tự nhận mình là Phùng Hưng'));
check('Có cụm nhận diện đảo vai',profile.roleplay?.identityGuard?.forbiddenNpcPhrases?.vi?.includes('Tôi là Phùng Hưng'));
check('Có thoại dự phòng đúng vai',profile.roleplay?.identityGuard?.fallbackNpcDialogue?.vi?.startsWith('Phùng Hưng'));
check('Baseline chỉ số mô phỏng đúng',JSON.stringify(profile.roleplay?.initialEvaluation)===JSON.stringify({military:3,diplomacy:2,publicSupport:3,logistics:2,politics:3,governance:2}));
...
const sampleHistory=[
  {role:'assistant',content:JSON.stringify({npcDialogue:'Phùng Hưng, xin cân nhắc kỹ.',feedback:'Lựa chọn cần cân bằng.',evaluation:{military:3,diplomacy:2,publicSupport:4,logistics:2,politics:3,governance:2},sourceIds:['S1'],choices:['A','B','C'],isGameOver:false,endReason:''})},
...
check('Panel Nguồn sử liệu còn container',html.includes('id="sourcesList"'));
...
check('Mobile intro scroll fix từ template mới',css.includes('MOBILE INTRO SCROLL FIX')&&css.includes('touch-action:pan-y')&&css.includes('-webkit-overflow-scrolling:touch'));
