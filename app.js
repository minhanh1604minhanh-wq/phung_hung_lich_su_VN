...
function renderSources(){const en=state.lang==='en';$('sourcesList').innerHTML=profile.sources.map(s=>{const url=typeof s.url==='string'?s.url.trim():'';const link=/^https?:\/\//i.test(url)?`<a href="${esc(url)}" target="_blank" rel="noopener noreferrer">${t('openSource')}</a>`:'';return `<article class="source-card" id="src-${s.id}"><h4>${esc(en?s.titleEn:s.title)}</h4><p><b>${esc(en?s.orgEn:s.org)}</b></p><p>${esc(en?s.typeEn:s.type)}</p>${link}</article>`}).join('')}
