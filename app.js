...
function analyticsPayload(eventType,{feature='',content='',metadata={},endedAt='',keepalive=false}={}){
  return {
    ...
    characterId:profile?.id||CFG.DEFAULT_CHARACTER_ID||'phung-hung',
    sessionMetadata:{characterId:profile?.id||'phung-hung'},
    metadata,
    keepalive
  };
}
...
function startAnalyticsSession(){
  ...
  void sendAnalyticsEvent('page_view',{feature:'Trang nhân vật',content:profile?.name||'Phùng Hưng'});
}
...
async function loadProfile(){
  const fallback=CFG.DEFAULT_CHARACTER_ID||'phung-hung';
...
function renderSources(){const en=state.lang==='en';$('sourcesList').innerHTML=profile.sources.map(s=>`<article class="source-card" id="src-${s.id}"><h4>${esc(en?s.titleEn:s.title)}</h4><p><b>${esc(en?(s.orgEn||s.org):s.org)}</b></p><p>${esc(en?(s.typeEn||s.type):s.type)}</p>${s.url?`<a href="${esc(s.url)}" target="_blank" rel="noopener">${t('openSource')}</a>`:''}</article>`).join('')}
function renderSuggestions(){const en=state.lang==='en';const make=(arr,target,input)=>$(target).innerHTML=(Array.isArray(arr)?arr:[]).map(x=>`<button class="chip" type="button" data-fill="${esc(x)}" data-input="${input}">${esc(x)}</button>`).join('');make(profile.qaSuggestions?.[en?'en':'vi'],'qaSuggestions','qaInput');make(profile.whatifSuggestions?.[en?'en':'vi'],'whatifSuggestions','whatifInput')}
