...
function profile(id='phung-hung'){
...
app.get('/health',(_,res)=>res.json({ok:true,character:'phung-hung',aiReady:Boolean(openai),analyticsReady:Boolean(ANALYTICS_API_URL&&ANALYTICS_INGEST_KEY)}));
...
async function forwardAnalyticsEvent(body={}){
  ...
  const p=profile(body.characterId||'phung-hung');
  ...
  const payload={
    ...
    character:{
      slug:p.id||'phung-hung',
      nameVi:p.name||'Phùng Hưng',
      nameEn:p.nameEn||'Phung Hung'
    },
